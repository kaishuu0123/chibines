		...
		for _, cmd := range list.Commands() {
			if cmd.HasUserCallback() {
				...
				gl.DrawElementsBaseVertexWithOffset(gl.TRIANGLES, int32(cmd.ElementCount()), uint32(drawType),
					uintptr(uint(cmd.IndexOffset())*uint(indexSize)), int32(cmd.VertexOffset()))
			}
...
func (renderer *OpenGL3) SetFontTexture(image *imgui.RGBA32Image) {
	...
	imgui.CurrentIO().Fonts().SetTextureID(imgui.TextureID(renderer.fontTexture))
...
func (renderer *OpenGL3) LoadImage(image *image.RGBA) (imgui.TextureID, error) {
...
func (renderer *OpenGL3) ReleaseImage(textureId imgui.TextureID) {
	gl.BindTexture(gl.TEXTURE_2D, 0)
	handle := uint32(textureId)
	gl.DeleteTextures(1, &handle)
}

func (renderer *OpenGL3) CreateImageTexture(img *image.RGBA) (imgui.TextureID, error) {
	// Upload texture to graphics system
	var lastTexture int32
	var handle uint32
	gl.GetIntegerv(gl.TEXTURE_BINDING_2D, &lastTexture)
	...
	return imgui.TextureID(handle), nil
}
